...
let debug_pretyping = CDebug.create ~name:"pretyping" ()
...
struct
  ...
  let pretype_letin self (name, c1, t, c2) =
    ...
    let () = debug_pretyping (fun () -> Pp.(str "pretype_letin subst1 " ++ Termops.Internal.print_constr_env (GlobEnv.env env) sigma j.uj_val ++ cut () ++ Termops.Internal.print_constr_env env sigma j'.uj_type)) in
